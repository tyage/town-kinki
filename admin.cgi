...
#make_town（街レイアウトの更新）
#town_form（街レイアウト画面）
#parts_taiou_hiyou（パーツ＆キーワード対応表）
#admin_bbs（管理者BBS）
#itiran（住民一覧）
#kensaku（個人ログファイル検索）
#kozindata_henkou（個人データ変更）
#deletUsr（ユーザー消去）
#hukkatu（データ復活）
#hukkatu2（ステータス復活）
#hukkatu3（アイテム復活）
#ad_orosi_kousin（卸商品更新）
#depaato_kousin（デパート更新）
#prof_sakujo（プロフィール削除）
#as_prof_sakujyo（斡旋所プロフィール削除）
#bbs1_settei（BBS1の設定画面）
#bbs1_settei_do（BBS1の設定実行）
#mail_soshin（一斉メール）
#kekkon_sakujo（削除時の離婚）
#--------------------------#
...
#================指定ホストアクセス拒否==================
	...
	if ($host_kyuka eq 'yes'){
	
        if(-e $in_host){
		open(IN,"< $in_host") || &error("Open Error : dene2.cgi");
		eval{ flock (IN, 1); };
		$my_host1 = <IN>;
		close(IN);
		chomp $my_host1;
		if($okhost eq ""){$okhost = $my_host1;}
		}
		if($okhost){
		local($flag)=1;
...
exit;
	...
	if (!($in{'admin_pass'} eq $admin_pass && $in{'kanrisya_id'} eq $kanrisya_id)){&error("パスワードが違います。");}
...
EOM
	...
	$issei_f = './dat_dir/prezento.cgi';
	open(IN,"< $issei_f") || &error("Open Error : $issei_f");
	eval{ flock (IN, 2); };
	@pure = <IN>;
	close(IN);
  ...
  print <<"EOM";
			...
			<input type=hidden name=kanrisya_id value="$in{'kanrisya_id'}">
			<input type=hidden name=admin_pass value="$in{'admin_pass'}">
			<input type=hidden name=cou_k_id value="$kensaku_ID">
			<input type="text" name="kanrisyaname" size="30" value="不定期新聞ＫＩＮＫＩ">
			<textarea name=isseimail rows=5 cols=80></textarea><br>
			●プレゼント送付<br>
...
#---------------街レイアウトの更新-------------------#
sub make_town {
	if ($in{'admin_pass'} ne $admin_pass){&error("パスワードが違います");}
		&header(2);
...
#==========一覧画面を作成=============
	print <<"EOM";
	<center>
	<div align=center class=tyuu>登録者一覧（ランキングデータ）</div><br>
	<a href=$this_script?mode=itiran&sort_id=0&admin_pass=$in{'admin_pass'}><span class=sub3>[新規登録順]</span></a><img src=$img_dir/space.gif width=5 height=1>
	...
	<a href=$this_script?mode=itiran&sort_id=49&admin_pass=$in{'admin_pass'}><span class=sub3>[資産順]</span></a><img src=$img_dir/space.gif width=5 height=1>
	<a href=$this_script?mode=itiran&sort_id=1&admin_pass=$in{'admin_pass'}><span class=sub3>[名前順]</span></a><img src=$img_dir/space.gif width=5 height=1>
	<a href=$this_script?mode=itiran&sort_id=38&admin_pass=$in{'admin_pass'}><span class=sub3>[最後の食事順]</span></a><img src=$img_dir/space.gif width=5 height=1>
	<a href=$this_script?mode=itiran&sort_id=28&admin_pass=$in{'admin_pass'}><span class=sub3>[ホスト順]</span></a><img src=$img_dir/space.gif width=5 height=1>
	</center>
	
	<table border=0 width=95% align=center cellspacing="1" cellpadding="5">
	<tr><td colspan=10>●名前をクリックするとその人の個人データにアクセスできます。またその画面で「ユーザー削除」「各種データの修正」「お金の振り込み（減金）」「メール送信」等が行えます。<br>※ブラックリストとは二重登録者として記録されたユーザーです。<br>※このリストの数字はすべてデータ保存時のもので、今現在のデータとは異なります。
	<form method="POST" action="$this_script">
	<input type=hidden name=mode value="kensaku">
	<input type=hidden name=command value="kozin_file">
	<input type=hidden name=kanrisya_id value="$in{'kanrisya_id'}">
	<input type=hidden name=admin_pass value="$in{'admin_pass'}">
	ID番号 <input type=text name=k_id size="6">こちらでIDを入力して個人データを開くこともできます。
	</form>
	...
	<input type=hidden name=kanrisya_id value="$in{'kanrisya_id'}">
	<input type=hidden name=admin_pass value="$in{'admin_pass'}">
	名前 <input type=text name=name size="20">こちらで名前を入力して個人データを開くこともできます。
	</form>
...
#koko2009/01/25
	open(AT,"< atakka.cgi") || &error("Open Error : atakka.cgi");
	eval{ flock (AT, 1); };
	@tem_atakku = <AT>;
	close(AT);
	foreach (@tem_atakku){
		($ma_addr,$ma_host,$ma_tim,$ma_kaisu) = split(/<>/);
		print "●$ma_addr,$ma_host,$ma_tim,$ma_kaisu ";
	}
	print "<br>\n";
#end
#kokoend #koko2008/05/24
...
<br><div align=center>総登録者数：$total_count人　男性：$m_count人　女性：$f_count人<br><br>
</div>
	</body></html>
EOM
...
EOM
		...
		$ima_item_file = "./member/$kensaku_ID/mono.cgi";
		open(MONO,"< $ima_item_file") || &error("ログファイル（$ima_item_file）が開けません");
		eval{ flock (MONO, 1); };
		@ima_mono = <MONO>;
		close(MONO);
		foreach(@ima_mono){
			&syouhin_sprit($_);
			$imamotimono .= "[ $syo_syubetu,$syo_hinmoku,$syo_taikyuu ]";
		}
#==========データ復活の場合=============
		...
		$my_item_file = "./member/"."$kensaku_ID"."backup/backup_mono.cgi";
		open(MONO,"< $my_item_file") || &error("ログファイル（$my_item_file）が開けません");
		eval{ flock (MONO, 1); };
		@my_mono = <MONO>;
		close(MONO);
		foreach(@my_mono){
			&syouhin_sprit($_);
			$mymotimono .= "[ $syo_syubetu,$syo_hinmoku,$syo_taikyuu ]";
		}
...
<form method="POST" action="$this_script">
<input type=hidden name=mode value="hukkatu2">
<input type=hidden name="k_id" value="$kensaku_ID">
<input type=hidden name=kanrisya_id value="$in{'kanrisya_id'}">
<input type=hidden name=admin_pass value="$in{'admin_pass'}">
<input type=submit value="個人データのみ復活させる">
</form>
$mymotimono
<form method="POST" action="$this_script">
<input type=hidden name=mode value="hukkatu3">
<input type=hidden name="k_id" value="$kensaku_ID">
<input type=hidden name=kanrisya_id value="$in{'kanrisya_id'}">
<input type=hidden name=admin_pass value="$in{'admin_pass'}">
<input type=submit value="個人アイテムデータのみ復活させる">
</form>
EOM
...
<option value="off">隠す</option>
</select>
<input type="submit" value="街に入る" onMouseDown='bzc()'>
</form></center><br>
EOM
	...
	<input type=text size=3 name="okurisuu" value="1">回（日）分を送る。<br>
...
#==========個人データ修正=============
	...
	&unlock;
	&message("データ修正をしました。","itiran","admin.cgi");
	&kensaku;
}
...
#==========プロフィール＆結婚斡旋所削除処理=============
	...
	&message("$in{'name'}さんを削除しました。","itiran","admin.cgi");
}
...
#==========フォルダー内のbackup_がついたファイル名を取得してバックアップデータを復活=============
	...
	&unlock;
	&message("最後に保存したデータに切り替えました。","itiran","admin.cgi");
}

#---------------データ復活ステータス-------------------#
sub hukkatu2{
#koko2008/06/12
	$mae_st_file = "./member/"."$in{'k_id'}"."backup/backup_log.cgi";
	open(MY,"< $mae_st_file") || &error("ログファイル（$mae_st_file）が開けません");
	eval{ flock (MY, 1); };
	$my_log = <MY>;
	close(MY);
	$ato_st_file = "./member/$in{'k_id'}/log.cgi";
	open(MY,">$ato_st_file") || &error("ファイル（$ato_st_file）が開けません");
	eval{ flock (MY, 2); };
	print MY $my_log;
	close(MY);
	&message("最後に保存したlogデータに切り替えました。","itiran","admin.cgi");
}

#---------------データ復活アイテム-------------------#
sub hukkatu3 {
#koko2008/06/12
	$mae_item_file = "./member/"."$in{'k_id'}"."backup/backup_mono.cgi";
	open(MONO,"< $mae_item_file") || &error("ログファイル（$mae_item_file）が開けません");
	eval{ flock (MONO, 1); };
	@my_mono = <MONO>;
	close(MONO);
	$ato_item_file = "./member/$in{'k_id'}/mono.cgi";
	open(MONO,">$ato_item_file") || &error("ファイル（$ato_item_file）が開けません");
	eval{ flock (MONO, 2); };
	print MONO @my_mono;
	close(MONO);
	&message("最後に保存したmonoデータに切り替えました。","itiran","admin.cgi");
}
...
#==========卸フラグを１にしてメインタウンログを更新=============
	...
	&unlock;
	&message("卸商品を更新しました。","admin","admin.cgi");
}
...
sub depaato_kousin{
	...
	&message("デパート・食堂を更新します。","admin","admin.cgi");
}
...
EOM
	...
	●タイトル下のコメント<br>
	<input type=text name="come" size=120  value=$bbs1_come><br>
	●背景のスタイル設定<br>
...
sub mail_soshin {
	...
	foreach (@pure){
		&syouhin_sprit($_);
		if ($syo_hinmoku eq $in{'gift_souhu'}){
			$syo_kounyuubi = time ();
			
			if ($in{'gift_souhu'} eq 'クーポン'){
				$syo_coupon = 1;
				$syo_syubetu = 'ギフト';
				$cou_taikyuu = $syo_taikyuu;
			}
	...
	foreach $tmp0(@member){
		...
		$m_comment = "$in{'isseimail'}";
		if ($in{'gift_souhu'}){
			$m_comment .= "<br><br><div class=honbun2>『$in{'gift_souhu'}』が贈られてきました。</div>";
		}
		$new_mail = "受信<>$in{'kanrisyaname'}<>$m_comment<>$date2<>$date_sec<><><><><><>\n";
...
#==========相手の持ち物リストに商品を追加=============
		...
		$atta = 0;
		$aite_monokiroku_file="./member/$member_list[0]/mono.cgi";
		...
		$taikyu0 = $syo_taikyuu;
		$sybetu0 = $syo_syubetu;
		$hinmoku0 = $syo_hinmoku;
		$atta = 0;
		foreach $temp(@aite_item_hairetu){
			&syouhin_sprit($temp);
			if ($hinmoku0 eq $syo_hinmoku && $syo_coupon && !$atta){
				$syo_taikyuu += $cou_taikyuu;
				$atta = 1;
			}
			if($sybetu0 eq $syo_syubetu && $hinmoku0 eq $syo_hinmoku && !$atta){
				$syo_taikyuu += $taikyu0;
				$atta = 1;
			}
			&syouhin_temp;
		...
		if ($atta == 0){
			@new_aite_item_hairetu = (@aite_item_hairetu,$ageru);
		}
		open(ASP,">$aite_monokiroku_file") || &error("Write Error : $aite_monokiroku_file");
		eval{ flock (ASP, 2); };
		print ASP @new_aite_item_hairetu;
		close(ASP);
