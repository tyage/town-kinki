...
#e
	...
	#&motimono_kensa("胃弁当");
	if ($kensa_flag == 1){$hassei_rand = int(rand(3))+1;}
	else{$hassei_rand = int(rand(3))+1;}
	
	if ($hassei_rand ==1){
		$event_rand = int(rand(44))+1;
		...
		if ($event_rand == 1){
			...
			if (int(rand(2))+1 == 1){
				if ($money <= 0){
					$happend_ivent .= "<div class=purasu>●スリに遭いましたが、盗むお金が無かったので捨てぜりふを残して立ち去りました。</div>";
				}else{
					if($money < $syou_rand){$syou_rand=$money;}
					$money -= $syou_rand;
					$happend_ivent .= "<div class=mainasu>●スリに遭いました。<br>　  $syou_rand円盗まれました。</div>";
				}
			}else{
				$money += $syou_rand;
				$happend_ivent .= "<div class=purasu>●スリを捕まえました。<br>　  謝礼金$syou_rand円がもらえました。</div>";
			}
		}
			
		if ($event_rand == 2){
			$syou_rand = int(rand(10000))+1;
			$happend_ivent .= "<div class=purasu>●道でお財布を拾いました。<br>　  $syou_rand円入っていたのでやっぱりねこばばしました。</div>";
			$money += $syou_rand;
		}
			
		if ($event_rand == 3){
			if (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=purasu>●宣教師に出会い、親しげに話しかけられたうえ、英語を教えてくれました。<br>　  英語力３アップ！</div>";
				$eigo += 3;
				...
				$happend_ivent .= "<div class=mainasu>●英語の先生に見放されました。<br>　  英語力３ダウン！</div>";
				$eigo -= 3;
			}
		}
			
		if ($event_rand == 4){
			$happend_ivent .= "<div class=mainasu>●風邪を引いてしまいました。</div>";
			$byouki_sisuu = -15;
		}

		if ($event_rand == 5){
			$happend_ivent .= "<div class=mainasu>●NHKの集金がやってきました。<br>　  1000円払いました。</div>";
			$money -= 1000;
		}

		if ($event_rand == 6){
			$happend_ivent .= "<div class=purasu>●失恋してやけ食いしました。<br>　  体重が５kg増えました。</div>";
			$taijuu += 5;
		}

		if ($event_rand == 7){
			if (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=purasu>●文学に目覚めました。<br>　  国語力３アップ！</div>";
				$kokugo += 3;
			}else{
				$happend_ivent .= "<div class=mainasu>●パ\ソ\コンばかりして漢字を忘れてしまいました。<br>　  国語力３ダウン！</div>";
				$kokugo -= 3;
			}
		}
		...
		if ($event_rand == 8){
			if (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=purasu>●数学パズルで遊びました。<br>　  数学力３アップ！</div>";
				$suugaku += 3;
			}else{
				$happend_ivent .= "<div class=mainasu>●数字恐怖症になりました。<br>　  数学力３ダウン！</div>";
				$suugaku -= 3;
			}
		}
		...
		if ($event_rand == 9){
			if (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=purasu>●テレビで科学の番組を見ました。<br>　  理科の力が３アップ！</div>";
				$rika += 3;
			}else{
				$happend_ivent .= "<div class=mainasu>●理科の実験で試験管を割ってしまいました。<br>　  理科の力が３ダウン！</div>";
				$rika -= 3;
			}
		}
		...
		if ($event_rand == 10){
			if (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=purasu>●歴史の本を読みました。<br>　  社会の力が３アップ！</div>";
				$syakai += 3;
			}else{
				$happend_ivent .= "<div class=mainasu>●社会への関心が引き潮が引くように無くなっていきました。<br>　  社会の力が３ダウン！</div>";
				$syakai -= 3;
			}
		}
		...
		if ($event_rand == 11){
			if (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=purasu>●ピアノの練習をしました。<br>　  音楽の力が３アップ！</div>";
				$ongaku += 3;
			}else{
				$happend_ivent .= "<div class=mainasu>●飲み会で騒ぎすぎガラガラ声になりました。<br>　  音楽の力が３ダウン！</div>";
				$ongaku -= 3;
			}
			&motimono_kensa_ev("成り上がり");
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=purasu>●「成り上がり」を読んで音楽へのやる気がわいてきました。<br>　  音楽5アップ</div>";
				$ongaku += 3;
			}
		}
		...
		if ($event_rand == 12){
			if (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=purasu>●芸術に目覚めました。<br>　  美術の力が３アップ！</div>";
				$bijutu += 3;
			}else{
				$happend_ivent .= "<div class=mainasu>●美術の授業で居眠りをしてしまいました。<br>　  美術の力が３ダウン！</div>";
				$bijutu -= 3;
			}
		}
		...
		if ($event_rand == 13){
			...
			$happend_ivent .= "<div class=purasu>●ラッキーくじが当たりました！<br>　  $kuzi_rand万円ゲット！</div>";
			$money += $kuzi_gaku;
		}
			
		if ($event_rand == 14){
			$happend_ivent .= "<div class=purasu>●不思議に優しい気持ちにつつまれました。<br>　  LOVE度が５アップ！</div>";
			$love += 5;
		}

		if ($event_rand == 15){
			$happend_ivent .= "<div class=mainasu>●裸で寝ていたら体調を崩したようです。</div>";
			$byouki_sisuu = -8;
		}
		...
		if($bank <= 0){
				$happend_ivent .= "<div class=mainasu>●空き巣が入りましたが、「ちっ、ここには金がねぇ。。」<br>  と言って出て行きました。</div>";
			}elsif ($kensa_flag == 1){
			if (int(rand(5))+1 == 1){
			&motimono_kensa_ev2("防犯カメラ");
				$happend_ivent .= "<div class=mainasu>●空き巣が入りましたが、防犯カメラが壊されてました。<br>  通帳を盗られ、普通口座の預け入れ額が５分の１減ってしまいました。</div>";
				$bank2=int ($bank / 5);
				$bank -= $bank2;
			&kityou_syori("空き巣","$bank2","",$bank,"普");
				}else{
			$money+=100000;
				$happend_ivent .= "<div class=purasu>●空き巣が入りましたが、防犯カメラがあったので捕まえることができました。<br>　  （謝礼金がもらえました）</div>";
				}
				
				}elsif(int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=mainasu>●空き巣に遭いましたが近所の人が捕まえました。</div>";
				}else{
				$happend_ivent .= "<div class=mainasu>●空き巣に遭いました。<br>　  通帳を盗られ、普通口座の預け入れ額が５分の１減ってしまいました。</div>";
				$bank2=int ($bank / 5);
				$bank -= $bank2;
			&kityou_syori("空き巣","$bank2","",$bank,"普");
				}
		...
		if ($event_rand == 17){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=purasu>●おばあちゃんの知恵をまとめて出した本の印税が3000円入りました。</div>";
				$money=$money+3000;
			}elsif($myranded == 1){
				$happend_ivent .= "<div class=purasu>●出した本の印税が1000円入りました。</div>";
				$money=$money+1000;
			}else{
				$happend_ivent .= "<div class=purasu>●出した本の印税が500円入りました。</div>";
				$money=$money+500;
			}
		}
		...
		if ($event_rand == 18){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=purasu>●家にあったフラフープがハッピーな出来事を運んできたようです！<br>　  家の前に77777円落ちてました。</div>";
				$money=$money+77777;
			}elsif($myranded == 1){
				$happend_ivent .= "<div class=purasu>●家の前に5000円落ちてました。</div>";
				$money=$money+5000;
			}else{
				$happend_ivent .= "<div class=mainasu>●前に財布をねこばばした事がばれて逆に500円払いました。</div>";
				$money=$money-500;
			}
		}
		...
		if ($event_rand == 19){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=purasu>●車にひかれかけましたが『谷保天満宮のお守り』が守ってくれ、さらに謝罪費として3万をもらうことができました。</div>";
				$money=$money+30000;
			}elsif($speed <= 180){
				$happend_ivent .= "<div class=mainasu>●車にひかれて軽いケガを負いました。<br>　  入院費として３万円かかりました。</div>";
				$money=$money-30000;
			}else{
				$happend_ivent .= "<div class=purasu>●車にひかれかけましたが、軽いフットワークでかわしました。</div>";
			}
		}
		...
		if ($event_rand == 20){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=purasu>●家にあった「幸せを呼ぶ香水」が持ち金を倍にしてくれました！</div>";
				$money=$money * 2;
			}elsif($myranded == 1){
				$happend_ivent .= "<div class=purasu>●家の前に１万円落ちてました。</div>";
				$money=$money+100000;
			}else{
				$happend_ivent .= "<div class=mainasu>●パチンコで１万すってしまいました。。</div>";
				$money=$money-10000;
			}
		}
		...
		if ($event_rand == 21){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=purasu>●家にあった「バイアグラ」を使ってLOVE度＆エッチ度３アップ！</div>";
				$love += 3;
				$etti += 3;
			}elsif($myranded == 1){
				$happend_ivent .= "<div class=purasu>●友人がこっそりスッポンエキスを送ってきてくれました。<br>　  エッチ度３アップ！</div>";
				$etti += 3;
			}else{
				$happend_ivent .= "<div class=mainasu>●下痢気味でエッチにも力が入りません。。<br>　  エッチ度３ダウン。。</div>";
				$etti -= 3;
			}
		}
		...
		if ($event_rand == 22){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=mainasu>●地震が発生しました！！<br>　  が、損害保険のおかげで全額保障されました。</div>";
			}elsif (int(rand(2))+1 == 1){
				$happend_ivent .= "<div class=mainasu>●地震が発生しました！！<br>　  持ち金が３分の１になってしまいました。</div>";
				$syushitu = $money - int ($money / 3);
				$money = int ($money / 3);
				&news_kiroku("地震","$in{'name'}さんが、地震にあって$syushitu円損害が出ました。");
			}else{
				$happend_ivent .= "<div class=purasu>●資産運用に成功して持ち金が倍になりました。</div>";
				$syushitu = $money;
				$money = int ($money*2);
		...
		if ($event_rand == 23){
			...
			if ($kensa_flag == 1){
				if ($myranded == 1){
					$happend_ivent .= "<div class=purasu>●「ブラタンメンバーズカード」特典♪<br>　  現金1万円が当たりました！</div>";
					$money += 10000;
				}elsif ($myranded == 2){
					$happend_ivent .= "<div class=purasu>●「ブラタンメンバーズカード」特典♪<br>　  現金10万円が当たりました！</div>";
					$money += 100000;
				}elsif ($myranded == 3){
						$happend_ivent .= "<div class=purasu>●「ブラタンメンバーズカード」特典♪<br>　  現金50万円が当たりました！</div>";
					$money += 500000;
				}elsif ($myranded == 4){
					$happend_ivent .= "<div class=purasu>●「ブラタンメンバーズカード」特典♪<br>　  現金100万円が当たりました！</div>";
					$money += 1000000;
				}
			}else{
				$in{'town_no'}=int(rand(3));
				$happend_ivent .= "<div class=mainasu>●ＵＦＯにさらわれて$town_hairetu[$in{'town_no'}]へ飛ばされてしまいました！！</div>";
			}
		}
		...
		if ($event_rand == 24){
			if (int(rand(10))+1 == 1){
				...
				$happend_ivent .= "<div class=purasu>●宝くじが当たりました！<br>　  $kuzi_hyouzi万円ゲット！</div>";
				$money += $kuzi_gaku;
			}else{
				$happend_ivent .= "<div class=mainasu>●宝くじを3000円分購入しましたがやっぱり外れました。</div>";
				$money -= 3000;
			}
		}

		if ($event_rand == 25){
			$happend_ivent .= "<div class=mainasu>●大阪のおばちゃんに説教されてノイローゼになりました。<br>　  体重が1kg減りました。</div>";
			$taijuu -= 1;
		}
		...
		if ($event_rand == 26){
			...
			if ($my_rand == 1){
				$energy -= int($energy / 3);
				$happend_ivent .= "<div class=mainasu>●ストリートファイトの巻き添えで、身体パワーが１/３減らされた。</div>";
			}elsif ($my_rand == 2){
				$nou_energy -= int($nou_energy / 3);
				$happend_ivent .= "<div class=mainasu>●ストリートファイトの巻き添えで、頭脳パワーが１/３減らされた。</div>";
			}elsif ($my_rand == 3){
				$energy -= int($energy / 4);
				$nou_energy -= int($nou_energy / 4);
				$happend_ivent .= "<div class=mainasu>●ストリートファイトの巻き添えで、身体パワー・頭脳パワーが１/４減らされた。</div>";
			}else{
				$happend_ivent .= "<div class=purasu>●ストリートファイトが起こりそうなので逃げ出して事なきを得た。</div>";
			}
		}
		...
		if ($event_rand == 27){
			$energy += 1000;
			$nou_energy += 1000;
			$happend_ivent .= "<div class=purasu>●温泉の入浴券をもらって身体・頭脳パワーが1000回復しました。</div>";
		}
		...
		if ($event_rand == 29){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=purasu>●家にあった「幸せの星砂」が持ち金を倍にしてくれました！</div>";
				$money = int ($money*2);
			}else{
				$happend_ivent .= "<div class=purasu>●家の前に１万円落ちてました。</div>";
				$money += 10000;
		...
		if ($event_rand == 30){
			$my_rand = int(rand(5))+1;
			&coupon_get($my_rand);
			$happend_ivent .= "<div class=purasu>●クーポン券を $my_randつもらいました！</div>";
		}
        
		if ($event_rand == 31){
			&motimono_kensa_ev("ミグ25");
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=mainasu>●グレムリンに悪戯された。<br>　  ミグ25の耐久が$dsp_syo_taikyuuになった！</div>";
			}else{
				$happend_ivent .= "<div class=purasu>●自動販売機に100円入ってました。</div>";
				$money += 100;
			}
		}
		...
		if ($event_rand == 32){
			...
			if ($kensa_flag == 1){
				$happend_ivent .= "<div class=mainasu>●必要な品物を持っていました。</div>";
				$money += 100000;
			}else{
				$happend_ivent .= "<div class=purasu>●５０００円拾った。</div>";
				$money += 500000;
			}
		}
		...
		if ($event_rand == 33){
			...
			if ($my_rand == 1){
				$happend_ivent .= "<div class=purasu>●吉本の芸人に出会いました。<br>　  面白さが３アップ！</div>";
				$unique += 3;
			}elsif ($my_rand == 2){
				$energy -= int($energy / 3);
				$happend_ivent .= "<div class=mainasu>●奈良で鹿に踏まれて、身体パワーが１/３減らされました。</div>";
			}else{
				$energy -= int($energy / 2);
				$happend_ivent .= "<div class=mainasu>●琵琶湖でおぼれました。<br>　  身体パワーが１/２になり、救助代５万円を払いました</div>";
				$money=$money-50000;
			}
		}
		...
		if ($event_rand == 34){
			...
			if ($kensa_flag == 1){
				if ($myranded == 1){
				$happend_ivent .= "<div class=purasu>●サンタさんがやってきて人形と交換に持ち金を２倍にしてくれました！</div>";
				$money = int ($money*2);
				}else{
			$energy += 10000;
			$nou_energy += 10000;
				$happend_ivent .= "<div class=purasu>●サンタさんがやってきて身体・頭脳パワーを10000回復してもらいました。</div>";
				}
			}elsif($money <= 0){
				$money = 0;
				$happend_ivent .= "<div class=purasu>●徳政令が発令されました。</div>";
			}else{
				$happend_ivent .= "<div class=mainasu>ゴキブリだ！２０秒以内に潰せ！</div>";
				$goki="on";
			}
		}
		...
		if ($event_rand == 35){
			...
			if ($kensa_flag == 1){
				if ($myranded == 1){
				$happend_ivent .= "<div class=purasu>●４次元福袋の中から１億円が出てきました！！</div>";
					$money += 100000000;
				}elsif ($myranded == 2){
			$energy += 10000;
			$nou_energy += 10000;
				$happend_ivent .= "<div class=purasu>●４次元福袋の中に温泉券が１０枚入ってました。<br>　  身体・頭脳パワーが10000回復しました。</div>";
				}elsif ($myranded == 3){
			$my_rand2 = int(rand(15))+1;
			&coupon_get($my_rand2);
			$happend_ivent .= "<div class=purasu>●４次元福袋の中にクーポン券が $my_rand2枚入ってました！</div>";
				}
			}else{
				$happend_ivent .= "<div class=mainasu>●サンタさんを見たと騒いで罰金１万円を払わされました。</div>";
				$money=$money-10000;
			}
		}
		...
		$money_aku2=$money_aku*2;
			...
			&kifu2;
				...
				$happend_ivent .= "<div class=mainasu>●悪魔の笛を吹いたら$money_aku2円なくなりました。</div>";
				$money=$money-$money_aku2;
				}
			}else{
				$happend_ivent .= "<div class=mainasu>●悪魔に$money_aku円すられました。</div>";
		...
		$money_ten2=$money_ten*2;
			...
			&kifu3;
				...
				$happend_ivent .= "<div class=purasu>●天使の笛を吹いたら$money_ten2円持ち金が増えました。</div>";
				$money=$money+$money_ten2;
				}
			}else{
				$happend_ivent .= "<div class=purasu>●天使から$money_ten円もらいました。</div>";
		...
		if ($event_rand == 38){
			if($power >= 1000){
				if(int(rand(5))+1 == 1){
					...
					&get_itm2($my_rand);
					if($kensa_flag == 2){
						$happend_ivent .= "<div class=mainasu>●サンタを倒したが持ち物が多くて手に入れられなかった。</div>";
					}else{
						$happend_ivent .= "<div class=purasu>●サンタを倒して$syo_hinmoku0を手に入れた。</div>";
					}
				}else{
					$money=$money+10000;
					$happend_ivent .= "<div class=purasu>●サンタと戦って、10000円ゲットしました。</div>";
				}
			}else{
				$energy = 0;
				$happend_ivent .= "<div class=mainasu>●サンタと戦ったが負けて身体POWERが０になった</div>";
			}
		}
		
		if ($event_rand == 39){
			if ($energy >= 500 && $nou_energy >= 500){
				$happend_ivent .= "<div class=purasu>●仕事にとても集中できました。仕事経験地１０うｐ！</div>";
				$job_keiken = $job_keiken+10;
			}else{
				$happend_ivent .= "<div class=mainasu>●疲れていて仕事に集中できません。仕事経験地５ダウン</div>";
		...
		if ($event_rand == 40){
			...
			if(int(rand(5))+1 == 1){
				if($kensa_flag == 2){
					$happend_ivent .= "<div class=mainasu>●持ち物が多くて手に入れられなかった。</div>";
				}else{
					$happend_ivent .= "<div class=purasu>●$syo_hinmoku0を手に入れた。</div>";
				}
			}else{
				$money=$money-500;
				$happend_ivent .= "<div class=mainasu>●500円を川の中に落としてしまった！</div>";
			}
		}
        
		if ($event_rand == 41){
			$happend_ivent .= "<div class=purasu>●自動販売機のおつり取り忘れを発見！<br>	１０００円拾いました。</div>";
			$money=$money+1000;
		}
		...
		if ($event_rand == 42){
			...
			if ($kensa_flag == 1){
				if($myranded==1){
					$energy=$energy-10000;
					$happend_ivent .= "<div class=mainasu>●阪神タイガースの応援に行き巨人ファンと乱闘を起こしました。身体パワーダウン！</div>";
				}else{
					$nou_energy=$nou_energy+100000;
					$happend_ivent .= "<div class=purasu>●阪神タイガースに応援に行ったら快勝しました！<br>　  頭脳パワーうｐ！</div>";
				}
			}elsif($myranded==1){
				...
				$energy=$energy-5000;
				$happend_ivent .= "<div class=purasu>●タイガースの選手に出会って、特訓させられました！<br>　  身体パワーダウン、能\力うｐ！</div>";
			}else{
				$happend_ivent .= "<div class=mainasu>●道でこけた拍子に持ち金を半分川に落としてしまいました・・・</div>";
				$money=int($money / 2);
			}
		}

		if ($event_rand == 43){
			$happend_ivent .= "<div class=mainasu>●失恋して食べ物が喉を通らなくなりました。<br>　  5キロ痩せました。</div>";
			$taijuu -= 5;
		}
		...
		if($event_rand == 45){
			$happend_ivent .= "<div class=mainasu>●引ったくりにあい、持ち金が半分になりました。</div>";
			$money = int($money / 2);
		}
		
		if($event_rand == 46){
			$happend_ivent .= "<div class=mainasu>●銀行の口座から1/3が引き出されていました。</div>";
			$bank = int($bank * 2 / 3);
		}
...
sub aisatu {
    ...
    #二重、連続禁止
    if($in{'name'} eq "Eliwood" or $in{'name'} eq "雲雀"){
    	&error("二重投稿です");
    }
